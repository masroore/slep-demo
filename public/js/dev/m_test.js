...
function load_grafico(source,filter){
    ...
    //Obtengo arreglo con todas las fechas unicas de la busqueda
        data.map((o) => {o.tipoPagina!='Total' ? fechas.push((o.fecha.replaceAll('-','/'))) : null})
            ...
            switch(filter){
                case 'interactions':
                    data.map((o) => {
                        switch(o.tipoPagina){
                            case ('Twitter'):
                                f == o.fecha.replaceAll('-','/') ? tw_canvas_data+=o.likes : null
                                break
                            case ('Youtube'):
                                f == o.fecha.replaceAll('-','/') ? yt_canvas_data+=o.likes : null
                                break
                            case ('Facebook'):
                                f == o.fecha.replaceAll('-','/') ? fb_canvas_data+=o.likes : null
                                break;
                            case ('Instagram'):
                                f == o.fecha.replaceAll('-','/') ? ig_canvas_data+=o.likes : null
                                break;
                        }
                    })
                    break;
                case 'followers':
                    data.map((o) => {
                        switch(o.tipoPagina){
                            case ('Twitter'):
                                f == o.fecha.replaceAll('-','/') ? tw_canvas_data+=o.seguidores : null
                                break
                            case ('Youtube'):
                                f == o.fecha.replaceAll('-','/') ? yt_canvas_data+=o.seguidores : null
                                break
                            case ('Facebook'):
                                f == o.fecha.replaceAll('-','/') ? fb_canvas_data+=o.seguidores : null
                                break;
                            case ('Instagram'):
                                f == o.fecha.replaceAll('-','/') ? ig_canvas_data+=o.seguidores : null
                                break;
                        }
                    })
                    break;
                case 'viralization':
                    data.map((o) => {
                        switch(o.tipoPagina){
                            case ('Twitter'):
                                f == o.fecha.replaceAll('-','/') ? tw_canvas_data+=o.viralizacion : null
                                break
                            case ('Youtube'):
                                f == o.fecha.replaceAll('-','/') ? yt_canvas_data+=o.viralizacion : null
                                break
                            case ('Facebook'):
                                f == o.fecha.replaceAll('-','/') ? fb_canvas_data+=o.viralizacion : null
                                break;
                            case ('Instagram'):
                                f == o.fecha.replaceAll('-','/') ? ig_canvas_data+=o.viralizacion : null
                                break;
                        }
                    })
                    break;
        ...
        switch(source){
            case 'all':
                entfernen = new Chart(document.getElementById('graph-inter'), {
                    type: 'line',
                    data: {
                      labels: ['2021-01-01','2021-01-03','2021-01-06','2021-01-10','2021-01-14','2021-01-20'],
                      datasets: [{
                        label: 'Twitter',
                        ...
                        borderWidth: 2,
                        data: [5000,4000,3000,6000,2000,10000]
                      }, {
                        label: 'Youtube',
                        ...
                        pointHoverBackgroundColor: '#fff',
                        borderWidth: 2,
                        data: [2000,7000,1000,3000,8000,6000]
                      }, {
                        label: 'Facebook',
                        ...
                        borderWidth: 2,
                        //borderDash: [8, 5],
                        data: [1000,5000,6000,9000,5000,3000]
                      }, {
                        label: 'Instagram',
                        ...
                        borderWidth: 2,
                        //borderDash: [8, 5],
                        data: [3000,1000,10000,2000,10000,8000]
                      }]
                    },
                    options: options
                });
...
function load_AllStats(){
    ...
    if (window.location.href.indexOf("md") != -1){load_footerCards('all')}
    if (window.location.href.indexOf("md") != -1){load_datatable_autores('all')}
    load_words_cloud('all')

    if (window.location.href.indexOf("md") != -1){setSourceButtons(valueButton)}
    if (window.location.href.indexOf("md") != -1){removeValues(cards)}
...
function load_data(){
    ...
    if(original_data[0].nombre_campania){
        ...
        if (window.location.href.indexOf("md") != -1){load_totalCards()}
                    ...
                    $.get(url_request+idCampaña+'/instagramposts').done(function(igposts) {
                        ...
                        igposts.map((igpost)=>{
                            allData.push({
                                ...
                                fecha: date_and_time(new Date(igpost.date))[0],
                                hora:date_and_time(new Date(igpost.date))[1],
