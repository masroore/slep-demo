...
<html>
<head>
	<title>A.Slacker - Twitter API</title>
...
<div class="container">
    <h2>A.Slacker - Twitter API Intento 1</h2>
        ...
        <tbody>
            @if(!empty($data))
                @foreach($data as $llave => $valor)
                    <tr>
                        <td>{{ ++$llave }}</td>
                        <td>{{ $valor['statuses']['id'] }}</td>
                        <td>{{ $valor['statuses']['id'] }}</td>
                        <td>
                            @if(!empty($valor['statuses']['extended_entities']['media']))
                                @foreach($valor['statuses']['extended_entities']['media'] as $v)
                                    <img src="{{ $v['media_url_https'] }}" style="width:100px;">
                                @endforeach
                            @endif
                        </td>
                        <td>{{ $valor['statuses']['favorite_count'] }}</td>
                        <td>{{ $valor['statuses']['retweet_count'] }}</td>
                    </tr>
                @endforeach
            @else
                <tr>
