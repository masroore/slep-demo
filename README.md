...
``` bash
# clone the repo
$ git clone https://github.com/coreui/coreui-pro-laravel-admin-template.git my-project
...
> Instructions for CoreUI Pro Laravel admin template only. _Free and Vue.js versions have separate instructions._
...
* `$iconString` - a string containing valid CoreUI Icon name (kebab-case), ex. `cil-speedometer` or `cil-chart-pie`
...
* `$rolesString` - a string with list of user roles this menu element will be available, ex. `"guest,user,admin"`
* `$visibleName` - a string caption visible in sidebar
* `$iconString` - a string containing valid CoreUI icon name (kebab-case). For example: `cil-speedometer` or `cil-chart-pie`
...
To add __translations__ to your link or other menu element, call function `addTranslation()`:
```php
$this->addTranslation( $lang, $visibleString, $menuElementId);
```
`$lang` - short language identifier. Example: "us" or "fr"
`$visibleString` - a string caption visible in sidebar 
`$menuElementId` - id of a menu element
Example:
```php
$id = $this->insertLink('guest,user,admin', 'Colors', 'http://example.com');
$this->addTranslation( 'fr', 'Couleurs', $id);
```

__IMPORTANT__ - At the end of `run()` function, call `joinAllByTransaction()` and `insertAllTranslations()` functions:
```php
...
* name the item in each language,
...
#### Manage Languages
Allows you to add, edit, delete and set default menu language.
